...
// ── Registro manual de tiendas (superadmin) ───────────────────────────────────

// POST /admin/api/register-shop — vincular tienda con token manual (sin OAuth)
router.post('/api/register-shop', async (req, res) => {
  // Solo superadmin
  if (!req.session?.userId) return res.status(401).json({ error: 'No autorizado' });
  try {
    const requestingUser = await prisma.user.findUnique({ where: { id: req.session.userId } });
    if (!requestingUser || requestingUser.role !== 'superadmin') {
      return res.status(403).json({ error: 'Solo superadmin puede registrar tiendas manualmente' });
    }
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }

  const { domain, accessToken, userEmail } = req.body;
  if (!domain || !accessToken) {
    return res.status(400).json({ error: 'domain y accessToken son requeridos' });
  }
  ...
  try {
    // Validar el token llamando a la API de Shopify
    ...
    if (!shopRes.ok) {
      ...
      return res.status(400).json({
        error: shopRes.status === 401
          ? 'Token inválido o sin permisos para esa tienda'
          : `Shopify respondió ${shopRes.status}: ${errData?.errors || 'error desconocido'}`,
      });
    }
    ...
    await prisma.shop.upsert({
      where:  { domain: shopDomain },
      update: { accessToken, shopName: shopInfo.name || shopDomain, email: shopInfo.email || null },
      create: { domain: shopDomain, accessToken, shopName: shopInfo.name || shopDomain, email: shopInfo.email || null },
    });

    // Vincular al usuario si se proporcionó email
    if (userEmail) {
      const targetUser = await prisma.user.findUnique({
        where: { email: userEmail.toLowerCase().trim() },
      });
      if (!targetUser) {
        return res.status(404).json({ error: `No existe un usuario con email ${userEmail}` });
      }
      await prisma.userShop.upsert({
        where:  { userId_shopDomain: { userId: targetUser.id, shopDomain } },
        update: {},
        create: { userId: targetUser.id, shopDomain },
      });
    }

    res.json({ ok: true, shopName: shopInfo.name || shopDomain, domain: shopDomain });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
